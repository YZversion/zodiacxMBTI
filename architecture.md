...
| Persona card | lookup `manifest.json` → `card_image_path` → `st.image` + text block; Aries→Sagittarius live (144 WebP cards) |
...
Scrapers, generative chart images, MBTI quiz banks, user tables, React SPAs, CI/cron/APM, Bloom-style scroll-video shell inside Streamlit. Per-sign 192 unique art is **incremental** (`mbti_tarot_cards/`); Aries→Sagittarius live in manifest (144 cards); remaining signs show text-only persona block until cards are composed.
