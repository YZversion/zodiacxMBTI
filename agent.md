...
7. **MAIN_SYSTEM:** sections 1–5 + **§6 延伸探索** (exactly 3 `###` items with short answers, not teaser CTAs). UI/HTML render §6 as native `<details>` folds (not `st.expander`). Length: §§1–5 ~1000–1300 chars; each §6 body 80–120. Edit prompts only when the human asks. **Do not** rewrite MAIN_SYSTEM from a small usage-stats snapshot (need ≥50 gens and a section with miss≥5, or qualitative interview notes) — see [SHIP.md](SHIP.md) 观察期. **Do not** slim prompts/XML or add truncating `max_tokens` to save API tokens without an explicit quality-approved change — see architecture LLM token note; measure with `tools/measure_llm_payload.py`.
...
16. **Persona cards:** show after summary; key = `{MBTI}_{sun_en}` from chart sun + form MBTI;「不确定」→ missing hint, no invented card. Runtime image = `card_image_path` → `assets/cards/webp/{id}.webp` from offline `tools/build_cards.py` (all 12 signs; 192 cards). No runtime Pillow compose. HTML export embeds the same composed card via `composed_card_data_uri`.
17. **Generate CTA:** fixed `key="generate_report"`; label is `解读 {古希腊星座}` from date preview (e.g. `解读 καρκίνος`), or「生成解读」when date missing. No MBTI in the label. Approximate sun under date is preview-only（以排盘为准）.
