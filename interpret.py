...
6. 全文使用简体中文。
7. 严格按下列二级标题输出（不要自创其他二级标题，尤其禁止「延伸探索」「想知道更多」或任何第 6 节）。第 4 节「关于你正在纠结的事」仅当用户提供了具体问题时报出，且该节应为全文最长；未提供则完全跳过第 4 节（标题也不要出现），其余章节编号仍为 1、2、3、5：
## 1. 核心性格画像
## 2. 金钱与事业风格
## 3. 关系与沟通风格
## 4. 关于你正在纠结的事
## 5. 当前阶段的一句话建议
8. 篇幅（按汉字计，含标点近似即可；宁短勿注水）：第 1 节 180–250 字；第 2、3 节各 220–300 字；第 4 节（若有）350–450 字；第 5 节 30–45 字。全文合计约 1000–1300 字（无第 4 节时约 700–950 字）。删掉重复论证，提高「准」的密度。
9. 钩子规则：第 1–4 节的第一句必须直接揭示一个具体的性格矛盾，让读者产生「为什么我会这样」的好奇心；该句必须同时是本节核心结论，后文只负责解释与验证。禁止与正文脱节的提问、预告、悬念或追加推荐；禁止在结尾生成「延伸探索」「想知道……吗」类伪 CTA。第 5 节不适用本条。
...
12. 成因禁令：严禁编造星盘数据之外的心理成因，包括但不限于童年经历、原生家庭、过往创伤；可以描述模式，不可以虚构模式的来历。
13. 出口规则：每一个负面或扎心的指认之后，必须以一个当事人可操作的视角、做法或自我提问收尾；报告整体基调是「看见并陪伴」，不是「审判」。
...
11. 禁止「延伸探索」「想知道更多吗」等未兑现的追问推荐。"""


def sanitize_main_report(report: str) -> str:
    """Drop model-invented §6 / 延伸探索 even if the prompt forbids it."""
    if not report:
        return ""
    return re.split(
        r"(?:\n|^)##\s*6[.、．]?\s*延伸探索",
...
def build_main_user_prompt(
    ...
    *,
    ...
    parts = [
        ...
        "要求：严格五节结构（有问题才写第 4 节），全文约 1000–1300 字；不要写第 6 节或「延伸探索」。",
        "",
        "【前置说明】",
    ]
